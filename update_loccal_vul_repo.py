    ...
    while lib is not None:
        ...
        if not os.path.exists(origin + paths[0] + '/' + paths[1] + '/' + paths[2]):
            try:
                ...
                k = open('./download_vul_fault_2023_5_25.txt', 'a')
                k.write(lib + '\n')
                k.close()
                lib = read(q)
                continue
        k = open('./already_download_vul_2023_5_25.txt', 'a')
        k.write(lib + '\n')
        k.close()
        lib = read(q)
...
def download_vul_libs():
    if not os.path.exists('./already_download_vul_2023_4_8.txt'):
        f = open('./already_download_vul_2023_5_25.txt', 'w')
        f.close()
    if not os.path.exists('./new_vul_repos'):
        os.mkdir('./new_vul_repos')
    if not os.path.exists('./download_vul_fault_2023_5_25.txt'):
        f = open('./download_vul_fault_2023_5_25.txt', 'w')
        f.close()
    f = open('lib_name_address.txt', 'r')
    lib_list = []
    for i in f.read().split('\n'):
        lib = i.split(' ')[-1].replace('E:/vul_repos/', '')
        lib_list.append(lib)
        print(lib)
    f.close()
    q = Queue()
...
def complete_update_vul_libs_thread(q):
    ...
    while lib is not None:
        try:
            ...
            n = n + 1
            if n == 5:
                k = open('./fault_update_vul_2023_6_13.txt', 'a')
                k.write(lib + '\n')
                k.close()
                n = 0
                lib = read(q)
            continue
        k = open('./already_update_vul_2023_6_13.txt', 'a')
        k.write(lib + '\n')
        k.close()
        lib = read(q)
        n = 0
...
def complete_update_vul_libs(origin):
    lib_list = []
    f = open('./already_update_vul_2023_6_13.txt', 'r')
...
if __name__ == '__main__':
    if not os.path.exists('./already_update_vul_2023_6_13.txt'):
        f = open('./already_update_vul_2023_6_13.txt','w')
        f.close()
    if not os.path.exists('./fault_update_vul_2023_6_13.txt'):
        f = open('./fault_update_vul_2023_6_13.txt', 'w')
        f.close()
