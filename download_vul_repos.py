    ...
    while lib is not None:
        ...
        if not os.path.exists(origin + paths[0] + '/' + paths[1] + '/' + paths[2]):
            try:
                ...
                k = open('./download_vul_repos_fault_2023_6_13.txt', 'a')
                k.write(lib + '\n')
                k.close()
                lib = read(q)
                continue
        k = open('./already_download_vul_repos_2023_6_13.txt', 'a')
        k.write(lib + '\n')
        k.close()
        lib = read(q)
...
if __name__ == '__main__':
    if not os.path.exists('./already_download_vul_repos_2023_6_13.txt'):
        f = open('./already_download_vul_repos_2023_6_13.txt', 'w')
        f.close()
    if not os.path.exists('./new_libs_location'):
        os.mkdir('./new_libs_location')
    if not os.path.exists('./download_vul_repos_fault_2023_6_13.txt'):
        f = open('./download_vul_repos_fault_2023_6_13.txt', 'w')
        f.close()
    f = open('./need_download_vul_repos.txt', 'r')
